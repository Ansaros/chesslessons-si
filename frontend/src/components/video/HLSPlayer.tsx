...
export function HLSPlayer({ hlsUrl, poster, hlsSegments }: HLSPlayerProps) {
  ...
  const createHLSPlayer = useCallback((retryCount = 0) => {
    ...
    class PresignedUrlLoader extends FetchLoader {
    ...
    hls.on(Events.MANIFEST_PARSED, (event: string, data: ManifestParsedData) => {
      ...
      if (data.fatal) {
        ...
        switch (data.type) {
          case ErrorTypes.NETWORK_ERROR:
            ...
            if (data.details === ErrorDetails.MANIFEST_LOAD_ERROR) {
  ...
  }, [hlsUrl, hlsSegments, cleanup, setIsLoading, setError, setLoadingMessage]);
  ...
  useEffect(() => {
    ...
    return cleanup;
  }, [hlsUrl, hlsSegments, createHLSPlayer, cleanup]);
