...
export const VideosView = () => {
  ...
  const fetchVideos = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BACKEND_URL}/videos?${params.toString()}`
      );
  ...
  const filteredVideos = videos.filter((video) => {
    const matchesSearch =
      ...
      getAttributeValue(video.attributes, "description")
        .toLowerCase()
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <p className="text-slate-600">
            ...
            {filteredVideos.length === 1 ? "видео" : "видео"}
          </p>
        </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredVideos.map((video) => {
                ...
                const instructor = getAttributeValue(
                  video.attributes,
                  "instructor"
                );
                ...
                const description = getAttributeValue(
                  video.attributes,
                  "description"
                );
