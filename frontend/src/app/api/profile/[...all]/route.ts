...
async function forwardToBackend(endpoint: string, options: RequestInit) {
  try {
    console.log(`Forwarding to backend: ${NEXT_PUBLIC_API_BACKEND_URL}/${endpoint}`);
    ...
    const axiosConfig = {
      ...
      url: `${NEXT_PUBLIC_API_BACKEND_URL}/${endpoint}`,
      headers: headers,
      data: options.body,
    };
...
export async function GET(
  ...
  // Determine if this is a profile or admin/attribute request
  let endpoint = route;
  if (route === "chess-levels") {
    endpoint = "admin/attribute/types";
  } else {
    endpoint = `profile${route ? `/${route}` : ""}${queryString}`;
  }
...
export async function POST(
  ...
  return forwardToBackend(`profile/${route}`, {
    method: "POST",
    headers: {
...
export async function PUT(
  ...
  return forwardToBackend(`profile${route ? `/${route}` : ""}`, {
    method: "PUT",
    headers: {
      "Content-Type": contentType,
      "Authorization": req.headers.get("Authorization") || "",
